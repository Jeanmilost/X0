...
//------------------------------------------------------------------------------
// class X_System_Tools - objective-c
//------------------------------------------------------------------------------
@implementation X_SystemTools
//------------------------------------------------------------------------------
+ (NSString*) GetAppName
{
...
+ (NSString*) GetMemoryStr
{
    ...
    if (memory >= 1)
        // return memory in KB
        return [[[NSString alloc]initWithFormat:@"%d KB", memory]autorelease];
...
+ (BOOL) CreateDir :(NSString*)pDirName :(BOOL)createIntermediate
{
    ...
    if (![pFileManager createDirectoryAtPath:pDirName
                       ...
                       error:&pError])
    {
        M_LogErrorT("Create dir - FAILED - " << [X_StringTools NSStrToWStr:[pError localizedFailureReason]]
...
//------------------------------------------------------------------------------
+ (unsigned) CountFiles :(NSString*)pDirName
{
    if (![self DirExists:pDirName])
        return 0;
    ...
    {
        M_LogErrorT("Count files - FAILED - " << [X_StringTools NSStrToWStr:[pError localizedFailureReason]]
                    << " - " << [X_StringTools NSStrToWStr:pDirName]);
        return 0;
    }
...
+ (BOOL) DeleteFile :(NSString*)pFileName
{
    ...
    if ([pFileManager isReadableFileAtPath:pFileName])
        ...
        {
            M_LogErrorT("Remove file - FAILED - " << [X_StringTools NSStrToWStr:[pError localizedFailureReason]]
